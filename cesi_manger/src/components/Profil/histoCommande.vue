  ...
  <div v-if="connectedClient != false" id="histo-to-print">
    ...
    <div>
      <li v-for="item in lastCommandes" :key="item.ref">
        Commande n° {{ item.ref }} : {{ item.product }} x{{ item.quantity }} -
        {{ item.priceTotal }}
      </li>
      <button class="btn btn-primary" @click="getPDF()">
        Télécharger en PDF
      </button>
    </div>
  </div>
  ...
  data() {
    return {
      lastCommandes: [
        ...
        {
          ref: 'R3',
          product: 'Le festin africain',
          quantity: '10',
          priceTotal: '10',
        },
        {
          ref: 'R4',
          product: 'Pâtes carbonara',
          quantity: '3',
          priceTotal: '45',
        },
      ],
